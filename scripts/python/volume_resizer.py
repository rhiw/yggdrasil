...
def main():
    ...
    if get_usage_ratio(mount_point) < ratio:
        logger.info("{0} usage ration is less than {1}. Quitting".format(mount_point, ratio))
        return

    logger.info("Proceeding with extending media volume")

    volume_id = create_new_volume()
    new_device_name = increment_partition_name(get_sorted_partitions[-1])
...
def get_usage_ratio(mount):
    ...
    logger.info("Usage ratio: %d", ratio)
    return ratio
